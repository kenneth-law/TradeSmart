...
class PortfolioManager:
    ...
    def calculate_position_size(self, ticker, score, price, volatility, volume, sector, beta):
        ...
        # No position if edge is too small
        if abs(expected_edge) < 0.2:
            return 0, 0, "Insufficient edge"
        
        # Base position size on edge and volatility (higher edge = larger position, higher vol = smaller position)
        # Kelly criterion-inspired sizing
        kelly_fraction = expected_edge / (volatility / 100)
    ...
    def get_position_recommendations(self, stock_scores, market_data=None):
        ...
        # Process new position candidates
        # Sort by score descending
        candidates = sorted([s for s in stock_scores if s['ticker'] not in self.positions and s.get('day_trading_score', 0) >= 70],
                           key=lambda x: x.get('day_trading_score', 0), reverse=True)
        
        # Calculate available capital for new positions
        available_capital = self.current_capital * 0.8  # Keep some cash reserve
        ...
        for candidate in candidates:
            ...
            dollar_amount, shares, reason = self.calculate_position_size(
                ticker, score, price, volatility, volume, sector, beta
            )
            
            if shares > 0:
                # Check if we have enough capital
                if dollar_amount <= available_capital:
                    recommendations['buy'].append({
                        'ticker': ticker,
                        'shares': shares,
                        'estimated_cost': shares * price,
                        'reason': f"New position with score {score}: {reason}"
                    })
