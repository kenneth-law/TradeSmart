...
class TradingSystem:
    ...
    def generate_watchlist(self, ranked_stocks, max_stocks=20, min_score=60):
        ...
        # If no stocks meet the minimum score, take the top scoring ones
        if not candidates and ranked_stocks:
            log_message(f"No stocks meet minimum score of {min_score}, using top {max_stocks} stocks instead")
            candidates = sorted(ranked_stocks, key=lambda x: x.get('day_trading_score', 0), reverse=True)
        else:
            # Sort by score (if we already have candidates)
    ...
    def run_complete_workflow(self, tickers, use_ml=True, execute_trades=False):
        ...
        # Step 9: Calculate individual stock returns from simulation
        log_message("Calculating individual stock returns...")
        strategy = 'ml' if use_ml else 'technical'
        ...
        for stock in self.watchlist:
            try:
                # Run individual backtest for each stock
                individual_result = self.run_backtest(
                    tickers=[stock['ticker']],
                    strategy=strategy,
                    start_date=start_date,
                    end_date=end_date
                )
                # Store the return percentage in the stock data
                stock['return_percentage'] = individual_result.total_return
            except Exception as e:
                log_message(f"Error calculating return for {stock['ticker']}: {str(e)}")
                stock['return_percentage'] = 0.0
